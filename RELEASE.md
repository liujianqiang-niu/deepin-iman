...
- **新建聊天按钮**：将原 AI 面板底部的「清屏」按钮改为面板标题栏右上角的「+」图标按钮，提示「新建聊天」，点击后清空当前对话，交互方式参考主流 AI 聊天客户端（ChatGPT、Claude 等）
- **Enter 发送提问**：在 AI 问答输入框中按 Enter 键即可发送提问；Shift+Enter 换行，支持多行输入
- 输入框占位提示同步更新，提示可用 Enter 发送

### v1.0.0

- AI 驱动翻译，本地缓存优先 + 预设中文包降级
- AI 使用样例生成，按 section 区分 C 代码 / 命令行
- VSCode 风格编辑器分栏面板，面板可弹出为独立窗口
- 数据管理：清空翻译缓存/浏览历史/收藏/索引
- 收藏与浏览历史批量删除
- 移除死代码与 glibc-doc 依赖
