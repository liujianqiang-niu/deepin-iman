// src/service/ai/QwenProvider.cpp
...
static constexpr const char* API_URL = "https://dashscope.aliyuncs.com/compatible-mode/v1/chat/completions";
static constexpr const char* MODEL = "qwen-max";

QwenProvider::QwenProvider(QObject* parent) : IAiProvider(parent) {
}

void QwenProvider::chat(const AiRequest& req,
                         std::function<void(const AiChunk&)> onChunk,
                         std::function<void(const AiResult&)> onDone,
                         std::function<void(const QString&)> onError) {
    if (!isConfigured()) {
        onError("通义千问 API key 未配置");
        return;
    }
