// src/service/ai/GlmProvider.cpp
...
static constexpr const char* API_URL = "https://open.bigmodel.cn/api/paas/v4/chat/completions";
static constexpr const char* MODEL = "glm-4-plus";

GlmProvider::GlmProvider(QObject* parent) : IAiProvider(parent) {
}

void GlmProvider::chat(const AiRequest& req,
                        std::function<void(const AiChunk&)> onChunk,
                        std::function<void(const AiResult&)> onDone,
                        std::function<void(const QString&)> onError) {
    if (!isConfigured()) {
        onError("智谱 GLM API key 未配置");
        return;
    }
